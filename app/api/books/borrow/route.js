...
export async function POST(req) {
  try {
    ...
    console.log("Active books:", count)
        
    if (count >= 3) {
      return NextResponse.json({ error: "Максимум 3 книгиhjol" }, { status: 400 })
    }
        
    const result = await pool.query(
      "INSERT INTO borrows (book_id, user_id, deadline) VALUES ($1, $2, CURRENT_DATE + INTERVAL '14 days') RETURNING id, deadline", 
      [bookId, userId]
    )
